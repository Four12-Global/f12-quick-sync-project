/*********************************************************************
 * airtable-helpers.ts  –  shared utilities for Quick-Sync scripts
 * ---------------------------------------------------------------
 * Usage in a table-specific entry file (e.g. series-sync.ts):
 *
 *   import { quickSync } from './lib/airtable-helpers';
 *
 *   quickSync({
...
export interface QuickSyncConfig {
  ...
  fieldMap:        FieldMap;
  envEndpoints:    Record<string, string>;   // prod / staging / …
  allowedStatuses?: string[];               // default below
...
export function fieldsToFetch(map: FieldMap): string[] {
  ...
  s.add('wp_id').add('series_sku').add('series_title'); // generic fallbacks
  return [...s];
}
  ...
  for (const src in map) {
    ...
    if (typeof mapping === 'string') {
      ...
      if (mapping === 'post_status') {
        ...
        fields[mapping] = coerceValue(raw);
      }
      continue;
    }
...
export async function quickSync(cfg: QuickSyncConfig) {
  ...
  const fieldsNeeded = fieldsToFetch(fieldMap);
  ...
  fields.sku        ??= record.getCellValueAsString('series_sku');
  fields.post_title ??= record.getCellValueAsString('series_title') || record.name;
  ...
  const data = await postToWp(wpUrl, authB64, payload);
